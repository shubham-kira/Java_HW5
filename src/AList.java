...
class AList<T> implements ListInterface<T> {
    ...
    */
    public void reverse()
    {
    	...
    	/*Keep swapping the elements from the first and the last positions
    	 * till we reach the middle of the array*/
    	for(int i=1;i<=numberOfEntries/2;i++)
    	{
    		T temp;
    		temp=list[numberOfEntries-i+1];
    		list[numberOfEntries-i+1]=list[i];
    		list[i]=temp;
    	}
    }
    ...
    */
    public void cycle()
    {
    	...
    	T temp=list[1];
    	int i;
    	//Copy each element to it's preceding position 
    	for(i=2;i<=numberOfEntries;i++)
    		list[i-1]=list[i];
    	//Copy the value of the first element to the last position
